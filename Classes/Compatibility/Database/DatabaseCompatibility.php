    ...
    public static function getDatabaseConnection()
    {
        if (version_compare(TYPO3_version, '8.0', '<')) {
            ...
            if (self::$databaseConnection === null) {
                ...
                self::$databaseConnection->setDatabaseName(
                    $GLOBALS['TYPO3_CONF_VARS']['DB']['Connections']['Default']['dbname'] ?? ''
                );
                self::$databaseConnection->setDatabaseUsername(
                    $GLOBALS['TYPO3_CONF_VARS']['DB']['Connections']['Default']['user'] ?? ''
                );
                self::$databaseConnection->setDatabasePassword(
                ...
                if (isset($GLOBALS['TYPO3_CONF_VARS']['DB']['Connections']['Default']['unix_socket'])) {
                    self::$databaseConnection->setDatabaseSocket(
                        $GLOBALS['TYPO3_CONF_VARS']['DB']['Connections']['Default']['unix_socket']
                    );
                }
                self::$databaseConnection->setDatabaseHost($databaseHost);

                self::$databaseConnection->debugOutput = $GLOBALS['TYPO3_CONF_VARS']['SYS']['sqlDebug'];
